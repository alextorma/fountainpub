...
export var GeneratePdf = function (outputpath: string, config: FountainConfig, exportconfig : ExportConfig, parsedDocument: any, progress?: { report: (value: { message?: string; increment?: number; }) => void; } ): Promise<any> {
    return new Promise((resolve, reject) => {
        try {
            if(progress && typeof progress.report === 'function') progress.report({message: "Converting to individual lines", increment: 25});
            ...
            parsedDocument.lines = liner.line(parsedDocument.tokens, {
                print: print.print_profiles[config.print_profile || "usletter"],
            ...
            if(outputpath=="$STATS$") 
                resolve(pdfmaker.get_pdf_stats(pdf_options));
