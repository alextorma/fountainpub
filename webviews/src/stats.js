...
// Responsive navigation functionality for webview
function initResponsiveNavigation() {
    // Sync navigation between sidebar and bottom nav
    const sidenavItems = document.querySelectorAll('.sidenav li[data-group]');
    const bottomnavItems = document.querySelectorAll('.bottom-nav li[data-group]');
    // Add click handlers for both navigation systems
    [...sidenavItems, ...bottomnavItems].forEach(item => {
        item.addEventListener('click', function () {
            handleNavClick(this.dataset.group);
        });
    });
    // Sync duration stats between normal and compact displays
    syncDurationValues();
}

function handleNavClick(group) {
    // Update active states in both navigations
    document.querySelectorAll('.sidenav li[data-group], .bottom-nav li[data-group]').forEach(item => {
        item.classList.toggle('active', item.dataset.group === group);
    });
    // Show/hide content sections
    document.querySelectorAll('#content > div[data-group]').forEach(section => {
        section.style.display = section.dataset.group === group ? 'block' : 'none';
    });
    // Trigger chart resize if needed (for the stats.js integration)
    if (typeof window.triggerChartResize === 'function') {
        setTimeout(() => window.triggerChartResize(), 100);
    }
}

function syncDurationValues() {
    // Sync values from main duration stats to compact version
    const totalElement = document.getElementById('durationStats-total');
    const actionElement = document.getElementById('durationStats-action');
    const dialogueElement = document.getElementById('durationStats-dialogue');
    const totalCompactElement = document.getElementById('durationStats-total-compact');
    const actionCompactElement = document.getElementById('durationStats-action-compact');
    const dialogueCompactElement = document.getElementById('durationStats-dialogue-compact');
    const totalFlexitemElement = document.getElementById('durationStats-total-flexitem');
    // Update compact displays (expanded 2-line view)
    if (totalElement && totalCompactElement) {
        totalCompactElement.textContent = totalElement.textContent;
    }
    if (actionElement && actionCompactElement) {
        actionCompactElement.textContent = actionElement.textContent;
    }
    if (dialogueElement && dialogueCompactElement) {
        dialogueCompactElement.textContent = dialogueElement.textContent;
    }
    // Update flexitem display (compact view)
    if (totalElement && totalFlexitemElement) {
        totalFlexitemElement.textContent = totalElement.textContent;
    }
}
...
// Expose functions globally for stats.js integration
window.syncDurationValues = syncDurationValues;
